...
export async function getMarkdownFile(fsPath: string, contents: string): Promise<void>  {
	...
	return utils.executeWithProgress(`${icon} ${path.basename(fsPath)} MARKDOWN`, async () => {
		...
		if (!choice) return;
		env = choice;
		let currentFolder = vscode.workspace.getWorkspaceFolder(vscode.Uri.file(fsPath))
		let markdownFile = path.join(currentFolder.uri.fsPath, ".configurations/markdown/tmp" , path.basename(fsPath).split(".")[0]+".md")
		await fs.ensureDir(path.dirname(markdownFile));
		await utils.writeFileWithSettings(markdownFile, contents);
		utils.logger.info(
			`${utils.icons.SUCCESS} ${utils.icons.MARKDOWN} ${path.basename(fsPath).split(".")[0]+".md"} generation from ${env.name} succeeded`
		);
